...
void updateState(const fdm_packet* pkt) {
    ...
    if (realtime_now > last_realtime + 1000*1e3) { // 500ms timeout
...
//    printf("[acc]%lf,%lf,%lf\n", pkt->imu_linear_acceleration_xyz[0], pkt->imu_linear_acceleration_xyz[1], pkt->imu_linear_acceleration_xyz[2]);
    ...
    x = (int16_t)pkt->imu_angular_velocity_rpy[0] * GYRO_SCALE;
    y = (int16_t)pkt->imu_angular_velocity_rpy[1] * GYRO_SCALE;
    z = (int16_t)pkt->imu_angular_velocity_rpy[2] * GYRO_SCALE; 
    // printf("[gyr]%d,%d,%d\n",(int16_t)pkt->imu_angular_velocity_rpy[0] ,(int16_t)pkt->imu_angular_velocity_rpy[1],(int16_t)pkt->imu_angular_velocity_rpy[2]);   
...
void pwmCompleteMotorUpdate(uint8_t motorCount) {
    ...
    pwmPkt.motor_speed[0] = motorsPwm[0] / outScale;
    pwmPkt.motor_speed[1] = motorsPwm[1] / outScale;
    pwmPkt.motor_speed[2] = motorsPwm[2] / outScale;
    pwmPkt.motor_speed[3] = motorsPwm[3] / outScale;
...
//    printf("[input]:%d,%d,%d,%d\n", motorsPwm_input[0], motorsPwm_input[1], motorsPwm_input[2], motorsPwm_input[3]);
