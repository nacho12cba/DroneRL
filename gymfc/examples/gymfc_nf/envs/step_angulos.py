...
import time
from .rewards_angulos import RewardEnvAngle

class StepEnvAngle(RewardEnvAngle): 
    ...
    def update_setpoint(self):
        if self.sim_time > self.next_pulse_time:
            if (self.angle_sp == np.zeros(3)).all():
                self.angle_sp = self.generated_input
    ...
    def reset(self):
        ...
        self.generated_input = self.sample_angle_target()
        return super().reset()
    ...
    def sample_angle_target(self):
        return self.np_random.normal(self.min_angle,self.max_angle,size=3)
