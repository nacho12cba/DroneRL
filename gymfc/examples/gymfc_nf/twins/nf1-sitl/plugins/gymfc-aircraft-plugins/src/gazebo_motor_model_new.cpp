...
void GazeboMotorModel::VelocityCallback(MotorCommandPtr &_cmd) {
  ...
  // else ref_motor_rot_vel_ = _cmd->motor(motor_number_)*max_rot_velocity_;
  else{
      if(_cmd->motor(motor_number_)==0)
        ref_motor_rot_vel_ = 0;
      else
        ref_motor_rot_vel_ = CommandTransferFunction(static_cast<double>(_cmd->motor(motor_number_)));
  } 
}
...
void GazeboMotorModel::UpdateForcesAndMoments() {
  ...
  double force = real_motor_velocity * real_motor_velocity * motor_constant_/300;
  // double force = real_motor_velocity * real_motor_velocity * motor_constant_/290;
