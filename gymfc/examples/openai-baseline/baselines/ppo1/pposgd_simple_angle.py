...
def traj_segment_generator(pi,pi_vel, env, horizon, stochastic, flight_log=None):
    t = 0
    ac_angle = env.action_space.sample() # not used, just so we have the datatype
    ac = np.zeros(4)
    ...
    ep_lens = [] # lengths of ...
    ep_number = 0
    delta_error_vel = np.zeros(3)
    last_vel = np.zeros(3)
    last_err_vel = np.zeros(3)
    state_angle = np.zeros(6)
    ...
    acs = np.array([ac_angle for _ in range(horizon)])
    prevacs = acs.copy()
    

    while True:
        prevac = ac_angle
        ac_angle, vpred = pi.act(stochastic, ob)
        ...
        if flight_log:
            flight_log.add(cur_ep_len, ob, rew, ac_angle, info)
        rews[i] = rew
...
def learn(env, policy_fn, *,
    ...
    with tf.Session() as sess:
        ...
        with tf.gfile.GFile(proto_buf_path, 'rb') as fid:
            ...
            pi_vel = PpoBaselinesPolicy(sess)
        ...
        ob = U.get_placeholder_cached(name="ob_angle")
        ...
        # ----------------------------------------
        seg_gen = traj_segment_generator(pi, pi_vel, env, timesteps_per_actorbatch,
